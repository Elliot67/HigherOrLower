...
function action() {
	...
	chrome.storage.local.get(['bdd'], function (result) {
		if (result.bdd == undefined) { // Si premiere fois de l'extension on cherche les données
			fetch('https://pastebin.com/raw/z7Z2MhfM')
	...
	if (document.getElementsByClassName('term-keyword__keyword')[0]) {
		...
		// Définir la valeur finale à ajouter dans la bdd
		...
		// Nouvelle fonction
		function majbdd(data) {
			chrome.storage.local.set({ bdd: data }, () => { console.log("BDD mise à jour"); });
		}
...
// Refaire la page des paramètres pour une pop up
